...
const AdminCategories = () => {
  ...
  return (
    <div>
      ...
      {showAddForm && (
        <form onSubmit={handleAddCategory} className="bg-white rounded-lg shadow p-4 sm:p-6 mb-6">
          <h3 className="text-lg sm:text-xl font-semibold mb-4 text-gray-900">Add New Category</h3>
      ...
      <div className="space-y-4">
        {categories.map((category) => (
          <div key={category.id} className="bg-white rounded-lg shadow p-4 sm:p-6">
            {editingId === category.id ? (
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-base sm:text-lg font-semibold text-gray-900">{category.name}</h3>
                  <p className="text-xs sm:text-sm text-gray-600 mt-1">{category.description}</p>
                  {category.image && (
                    <img
                      src={category.image}
                      alt={category.name}
                      className="w-16 sm:w-24 h-16 sm:h-24 object-cover rounded mt-2 border border-gray-300"
                    />
                  )}
                </div>
